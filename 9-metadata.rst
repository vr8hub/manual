...
The metadata makes heavy use of Schema.org metadata. The IDPF has an article on `integrating Schema microdata with the flat structure of OPF files <https://idpf.github.io/epub-guides/schema-org-integration/>`__ (but note that this article was written for an earlier version of epub, so some points might no longer apply).
...
#.	If a book title contains numbers or abbreviations, a :html:`<meta property="dcterms:alternative" refines="#title">` element is placed after the main title block, containing the title with expanded or alternate spelling to facilitate possible search queries.
...
#.	The :path:`production-notes.md` file contains any notes related to the ebook’s production that the producer thinks would be important to future editors of the ebook. For example, the producer might note that page scans were not available, so an editorial decision was made to add commas to sentences deemed to be transcription typos; or that certain archaic spellings were retained as a matter of prose style specific to the ebook.

#.	The :path:`production-notes.md` file is not present if there are no production notes.
...
#.	:html:`<dc:subject>` values are sourced from `Library of Congress Subject Headings <https://id.loc.gov/authorities/subjects.html>`__.
...
#.	SE subjects are included with one or more :html:`<meta property="schema:genre">` elements.

	.. code:: html

		<meta property="schema:genre">Fantasy</meta>
		<meta property="schema:genre">Philosophy</meta>
...
#.	:html:`<link href="<WIKIPEDIA_URL>" rel="schema:sameAs">` contains the English Wikipedia URL for the book. This element is not present if there is no English Wikipedia entry for the book.

#.	The source code repository is modeled as a :value:`schema:workExample` by the following block:

		.. code:: html

			<meta property="schema:workExample" id="vcs-repository">schema:workExample</meta>
			<meta property="rdf:type" refines="#vcs-repository">http://schema.org/SoftwareSourceCode</meta>
			<link href="<URL>" refines="#vcs-repository" rel="schema:codeRepository"/>

	where :value:`<URL>` is the URL for the source control repository for the ebook. For repositories hosted on GitHub, it is calculated by taking the string :html:`https://github.com/standardebooks/` and appending the `SE identifier </manual/VERSION/9-metadata#9.2>`__, without :html:`https://standardebooks.org/ebooks/`, and with forward slashes replaced by underscores.
...
#.	:html:`<meta property="belongs-to-collection" id="collection-N">` contains the name of the collection the ebook belongs to.
	...
	#.	If the ebook is part of a series under a different title, for example, if the ebook is a short story omnibus containing a short story that is the actual entry in the collection, then the element is further refined with a model of :value:`schema:hasPart` by the following block:
		...
		For example, this is how the omnibus of Edgar Allan Poe’s Short Fiction models the location of one of its constituent stories in a Greatest Books collection:
			...
			.. code:: html
				...
				<meta id="collection-1" property="belongs-to-collection">Encyclopædia Britannica’s Gateway to the Great Books</meta>
				<meta property="collection-type" refines="#collection-1">set</meta>
				<meta property="schema:hasPart" refines="#collection-1" id="collection-1-entry-1">schema:hasPart</meta>
				<meta property="rdf:type" refines="#collection-1-entry-1">http://schema.org/CreativeWork</meta>
				<meta property="schema:name" refines="#collection-1-entry-1">The Telltale Heart</meta>

#.	:html:`<meta property="schema:additionalType">http://schema.org/Collection</meta>` is present if the ebook is a collection of items which a reader may wish to search by item title. For example, this would include ebooks like a collection of short stories, or a collection of short works by an author from antiquity.
...
#.	The :html:`<meta property="schema:wordCount">` element contains an integer representing the ebook’s total word count, excluding some SE files like the colophon and Uncopyright.

#.	The :html:`<meta property="schema:educationalLevel">` element contains a decimal representing the computed Flesch reading ease for the book.
...
#.	:html:`<meta property="schema:alternateName" refines="#author">` contains the author’s full name, with any initials or middle names expanded, and including any titles. The first instance of this element *must* be the author’s full first name. If the author uses a pseudonym, then this should be the full pseudonym, not the author’s real name. This element is not included if the value is identical to :html:`<dc:creator>`.
...
#.	:html:`<link href="<WIKIPEDIA_URL>" rel="schema:sameAs" refines="#author">` contains the URL of the author’s English Wikipedia page. This element is not included if there is no English Wikipedia page.

#.	:html:`<link href="<URL>" refines="#author" rel="schema:sameAs">` contains the URL of the author’s `Library of Congress Names Database <https://id.loc.gov/authorities/names.html>`__ page. This element is not included if there is no LoC Names database entry.
	...
	.. tip::
		...
		If it’s not on Wikipedia, find it directly by visiting the `Library of Congress Names Database <https://id.loc.gov/authorities/names.html>`__.

#.	:html:`<meta property="role" refines="#author" scheme="marc:relators">` contains the `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ for the roles the author played in creating this book.
...
.. code:: html
	...
	<meta property="schema:alternateName" refines="#author">Anton Pavlovich Chekhov</meta>
	<meta property="alternate-script" refines="#author">Анто́н Па́влович Че́хов</meta>
	<link href="https://en.wikipedia.org/wiki/Anton_Chekhov" refines="#author" rel="schema:sameAs"/>
	<link href="https://id.loc.gov/authorities/names/n79130807.html" refines="#author" rel="schema:sameAs"/>
	<meta property="role" refines="#author" scheme="marc:relators">aut</meta>
...
#.	The `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ is :string:`trl`: :html:`<meta property="role" refines="#translator" scheme="marc:relators">trl</meta>`.

#.	Translators often annotate the work; if this is the case, the additional `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ :string:`ann` is included in a separate :html:`<meta property="role" refines="#translator" scheme="marc:relators">` element.
...
#.	The `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ is :string:`ill`: :html:`<meta property="role" refines="#illustrator" scheme="marc:relators">ill</meta>`.
...
#.	The `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ is :string:`art`: :html:`<meta property="role" refines="#artist" scheme="marc:relators">art</meta>`.
...
#.	The :html:`id` attribute of the :html:`<dc:contributor>` is the lowercase, URL-safe, fully-spelled out version of the `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__. For example, if the MARC relator tag is :string:`wpr`, the :html:`id` attribute would be :value:`writer-of-preface`.

#.	The `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ is one that is appropriate for the role of the additional contributor. Common roles for ebooks are: :string:`wpr`, :string:`ann`, and :string:`win`.
...
#.	The `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ is :string:`trc`: :html:`<meta property="role" refines="#transcriber-1" scheme="marc:relators">trc</meta>`.

#.	If the transcriber’s personal homepage is known, the element :html:`<link href="<URL>" refines="#transcriber-1" rel="schema:url">` is included, whose value is the URL of the transcriber’s homepage. The URL must link to a personal homepage only; no products, services, or other endorsements, commercial or otherwise.
...
#.	The `MARC relator tag <https://www.loc.gov/marc/relators/relacode.html>`__ is :string:`spn`: :html:`<meta property="role" refines="#sponsor" scheme="marc:relators">spn</meta>`.

#.	If the sponsor’s personal homepage is known, the element :html:`<link href="<URL>" refines="#sponsor" rel="schema:url">` is included, whose value is the URL of the sponsor’s homepage. Since sponsors may be corporations, linking to a corporate homepage is permitted; however linking to specific product or service is disallowed.
...
#.	If the producer’s personal homepage is known, the element :html:`<link href="<URL>" refines="#producer-1" rel="schema:url">` is included, whose value is the URL of the transcriber’s homepage. The URL must link to a personal homepage only; no products, services, or other endorsements, commercial or otherwise.

#.	The `MARC relator tags <https://www.loc.gov/marc/relators/relacode.html>`__ for the SE producer usually include all of the following:
